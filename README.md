...
Set `COPILOT_ENABLED=true` for the backend, phone, and watch builds to enable Conversation Copilot. On the phone, start a session, enter the conversation situation and one goal, then tap `Enable copilot` to grant `read:context`, `read:transcript`, and `act:audio` for that session. A watch `Ask copilot` tap automatically creates concise goal-based advice and sends it through the existing audio path. Set `COPILOT_MODE=mcp` only when an MCP host should generate the advice through `handle_copilot_request` instead.
